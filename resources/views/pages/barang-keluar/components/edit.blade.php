    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('createBarangMasukData', () =>  ({
                showQtyInput: [],
                selectedDistributor: Number("{{ $barangKeluar->dist_id }}"),
                additionalField: '',
                detailDistributor: null,
                distributor: null,
                detailBarangMasuk:null,
                async init(){
                    const detailBarangMasuk = await axios.get(`/transaksi/barang-keluar/detail/${id}`);
                ...
                async save(){
                    await axios.put(`/transaksi/barang-keluar/${id}`, new FormData(formEdit))
                        .then(() => {
                            Swal.fire({
                                title: "Berhasil",
                                icon: "success"
